...
base_image = Image.open("/home/dong/projects/sd/template_images/template.jpg")  # 원본 템플릿 이미지
mask_image = Image.open("/home/dong/projects/sd/template_images/template_mask_face.jpg")  # 마스크 (교체할 부분)
input_image = Image.open("/home/dong/projects/sd/input/input_face.jpg")  # ip-adapter 참조 이미지
...
ip_adapter = pipeline.load_ip_adapter("h94/IP-Adapter", subfolder="models", weight_name="ip-adapter-plus-face_sd15.safetensors")

# 프롬프트 설정
prompt = "mbas, Cartoons"
...
output = pipeline(
    ...
    ip_adapter_weight_type="linear",  # weight_type
    ip_adapter_combine_embeds="concat", # combine_embeds
    ip_adapter_start=0.0,          # start_at
    ip_adapter_end=1.0,            # end_at
    ip_adapter_embeds_scaling=1.0, # embeds_scaling
    guidance_scale=8.0,  # CFG 값 변경
